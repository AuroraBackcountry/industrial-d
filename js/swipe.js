...
async function loadQueue(){
  var units = [];
  try{
    const res = await fetch(REST+'/industrial_dating_members?select=name,title,bio,photo&order=created_at.desc',{headers:API_HEADERS});
    if(res.ok){
      (await res.json()).forEach(function(m){
        units.push({ key:slug(m.name), name:m.name, photo:m.photo||'', meta:m.title||'Unspecified unit', bio:m.bio||'' });
      });
    }
  }catch(e){ /* handled below */ }
  if(!units.length){
    deck.innerHTML='<div class="sdone"><span class="bigstamp">Deck offline</span><p>// COULD NOT PULL PERSONNEL FILES<br>// TRY A REFRESH</p></div>';
    return;
  }
